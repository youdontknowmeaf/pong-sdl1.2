...
typedef struct critical_struct
{
    volatile int lock;
    volatile int owner;
} CRITICAL_SECTION;
...
{
    if (sceKernelGetThreadId() == cs->owner)
    {
        cs->lock++;
        return;
    }
...
{
    static ALuint TicksPerMilliSecond = 0xffffffff;

    // Only do this once because it hurts
    if (TicksPerMilliSecond == 0xffffffff)
        TicksPerMilliSecond = (ALuint)(sceRtcGetTickResolution() * 0.001f);

    unsigned long long tl;
    sceRtcGetCurrentTick(&tl);
    tl = tl / TicksPerMilliSecond;
