...
int main(int argc, char *argv[]) {
	...
	maple_device_t *cont = maple_enum_type(0, MAPLE_FUNC_CONTROLLER); /*Dreamcast bullshit 4 controller.*/
	...
	while(Quit != 1) {
		...
		/*Alternative Event handling (dreamcast)*/	
		if(cont) {
                cont_state_t *state = (cont_state_t *)maple_dev_status(cont);
                if(state->buttons & CONT_DPAD_UP) PaddleLY -= 20;
                if(state->buttons & CONT_DPAD_DOWN) PaddleLY += 20;
                }
